...
// รับ availableAgents เข้ามาเพื่อให้เลือก Model ได้เหมือนหน้าแชทหลัก
export default function AiAssistantPanel({ onClose, availableAgents = [] }) {
  
  // ตั้งค่าเริ่มต้น: ถ้ามี agent ให้ใช้ตัวแรก, ถ้าไม่มีให้ใช้ Default
  ...
  // ฟังก์ชันส่งข้อความ (จำลองการตอบ)
    ...
    <div className="w-[320px] h-[550px] max-h-[80vh] bg-[rgba(32,41,59,0.95)] border border-[rgba(254,253,253,0.3)] backdrop-blur-xl rounded-3xl shadow-2xl flex flex-col overflow-hidden absolute bottom-24 right-5 z-40 animate-fade-in-up origin-bottom-right">
      ...
      <div className="p-4 border-b border-white/10 flex justify-between items-center bg-white/5 relative">
        ...
        <div className="relative" ref={dropdownRef}>
            ...
            >
                <div className="w-9 h-9 rounded-full bg-gradient-to-tr from-indigo-500 to-purple-600 flex items-center justify-center shadow-lg text-lg">
                    {activeAgent.emoji}
                </div>
        ...
        {/* ปุ่มปิด */}
        <button onClick={onClose} className="text-white/50 hover:text-white transition bg-white/5 p-1.5 rounded-lg hover:bg-red-500/20 hover:text-red-400">
          <X size={18} />
        </button>
      </div>
