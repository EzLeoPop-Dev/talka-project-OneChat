...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    // 1. Validation เบื้องต้น (ยังคงไว้)
    if (!username.trim()) {
      setErrorUser(true);
      hasError = true;
    ...
    <div className="relative flex justify-center items-center min-h-screen">
          ...
          <div className="w-[460px] bg-gradient-to-b from-[#ffffff] via-[#f7ebff] to-[#e6d6ff] rounded-3xl shadow-xl p-10 flex flex-col items-center relative">
            ...
            <form className="w-full space-y-6" onSubmit={handleSubmit}>
              ...
              <div className="relative">
                ...
                {errorUser && <p className="absolute text-red-500 text-xs left-3 top-full mt-1">โปรดกรอกชื่อผู้ใช้</p>}
              </div>
              ...
              <div className="pt-2 space-y-3">
                ...
                <Link href="/auth/register">
                  <button type="button" className="w-full bg-white text-purple-700 py-2.5 rounded-full hover:bg-gray-200 transition shadow-sm">สมัครสมาชิก</button>
                </Link>
              </div>
              ...
              <div className="flex flex-col items-center mt-4">
                ...
                <button type="button" onClick={() => alert("Login with Google")} className="w-12 h-12 rounded-full bg-white flex items-center justify-center shadow-md hover:scale-105 transition-transform duration-200">
      ...
      <AnimatePresence>
        {popup.show && (
          ...
          >
            <motion.div
              className={`relative w-[400px] min-h-[280px] bg-white/90 backdrop-blur-xl border border-white/60 shadow-2xl rounded-3xl p-8 flex flex-col justify-center items-center gap-6 text-center ${popup.success ? "shadow-purple-500/20" : "shadow-red-500/20"}`}
              initial={{ scale: 0.8, y: 20, opacity: 0 }}
              animate={{ scale: 1, y: 0, opacity: 1 }}
              exit={{ scale: 0.9, y: 20, opacity: 0 }}
            >
              <button className="absolute top-4 right-4 text-gray-400 hover:text-gray-600" onClick={closePopup}>
                <X size={20} />
              </button>
